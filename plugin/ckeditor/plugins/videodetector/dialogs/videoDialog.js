...
CKEDITOR.dialog.add( 'videoDialog', function( editor ) {
    return {
        title: 'Youtube, Vimeo, Dailymotion URL 또는 embed code를 입력해주세요.',
        minWidth: 400,
        minHeight: 100,
        contents: [
            {
                ...
                elements: [
                    {
                        type: 'text',
                        id: 'url_video',
                        label: 'Youtube, Vimeo, Dailymotion URL 또는 embed code',
                        validate: CKEDITOR.dialog.validate.notEmpty( "값을 입력해주세요" )
                    }
                ]
            }
        ],
        ...
        onOk: function(){
            ...
            remove_btn.setAttribute('value', '비디오 삭제');
            remove_btn.setAttribute('class', 'remove-videodetector');
            var space_br = new CKEDITOR.dom.element('br');
            p.append(remove_btn);

            editor.insertElement(p);
            editor.insertElement(space_br);
